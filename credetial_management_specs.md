...
Backends are compile-time gated and runtime probed:

- Keychain probe is compiled only on macOS builds.
- libsecret probe is compiled only when `pkg-config libsecret-1` is available.
- File backend is always compiled and acts as fallback.
...
- **Format (v1)**:
  ```json
  {
    "version": "1",
    "entries": [
      { "ref": "MyPostgres", "secret": "my_password" }
    ]
  }
  ```
- **Permissions**: file is created with mode `0600`. On every access (read or write), ownership and mode are validated strictly; if invalid, operations fail closed.
...
- **Strict subset of config.json** so we can reuse `catalog_load_from_file` without
  parser forks.
...
2. If it exists, verify it is user-owned regular file with mode `0600`.
3. If mode/ownership/type is invalid, fail the operation and print a clear error.
