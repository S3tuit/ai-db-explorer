...
// used to model the autogenerated json schemas for our tools
typedef struct {
  const char *name;
  const char *tool_json;
} BrespToolJsonDef;

#include "tool_defs.generated.inc"
...
/* Appends one tools/list result payload to the root JSON-RPC object.
 * It borrows 'sb' and serializes the static broker tool inventory.
 * Returns OK on success, ERR on invalid input or serialization failure.
 */
static AdbxStatus bresp_append_tools_list_result(StrBuf *sb) {
  if (!sb)
    return ERR;

  if (json_kv_obj_begin(sb, "result") != OK)
    return ERR;
  if (json_kv_arr_begin(sb, "tools") != OK)
    return ERR;
  for (size_t i = 0; i < ARRLEN(bresp_tool_defs); i++) {
    if (json_arr_elem_raw_json(sb, bresp_tool_defs[i].tool_json) != OK)
      return ERR;
  }
...
BrokerResponse *bresp_create_tools_list(const McpId *id) {
  ...
  bresp->kind = BRESP_KIND_TOOLS_LIST;
  return bresp;
}
...
AdbxStatus bresp_to_jsonrpc(const BrokerResponse *bresp, StrBuf *out_json) {
  ...
  case BRESP_KIND_CONN_PROFILES:
    ...
    break;
  case BRESP_KIND_TOOLS_LIST:
    if (bresp_append_tools_list_result(out_json) != OK)
      goto err;
    break;
  case BRESP_KIND_RELATION_INFO:
