...
static size_t bc_avail(const BufChannel *bc) {
    if (bc->buf.len < bc->rpos) return 0;
...
static ssize_t bufch_fill(BufChannel *bc) {
    ...
    if (bc->rpos > 0 && (bc->rpos >= (bc->buf.cap / 2) || bc->rpos >= BUFIO_READ_CHUNK)) {
        bufch_compact(bc);
    }
...
int bufch_read_n(BufChannel *bc, void *dst, size_t n) {
    if (bufch_ensure(bc, n) != YES) return ERR;
...
static int bufch_write_all_no_flush(BufChannel *bc, const void *src, size_t n) {
    if (!bc || !bc->ch) return ERR;
    if (!src && n != 0) return ERR;
    if (n == 0) return OK;
...
int bufch_write2v(BufChannel *bc,
    ...
    if (!bc || !bc->ch) return ERR;
    if ((!h && hlen != 0) || (!p && plen != 0)) return ERR;
    if (hlen == 0 && plen == 0) return OK;
    ...
    if (bufch_write_all_no_flush(bc, h, hlen) != OK) return ERR;
    if (bufch_write_all_no_flush(bc, p, plen) != OK) return ERR;
    if (bytech_flush(bc->ch) != OK) return ERR;
    return OK;
}
