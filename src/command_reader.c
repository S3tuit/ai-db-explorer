...
static int command_reader_make_sql(const unsigned char *stmt, size_t len,
                                   Command **out_cmd) {
  Command *cmd = (Command *)xcalloc(1, sizeof(*cmd));
  cmd->type = CMD_SQL;
  cmd->raw_sql = (char *)xmalloc(len + 1);
  if (len > 0)
    memcpy(cmd->raw_sql, stmt, len);
  cmd->raw_sql[len] = '\0';
  *out_cmd = cmd;
  return YES;
}
 ...
 * bytes from 'stmt'. */
static int command_reader_make_meta(const unsigned char *stmt, size_t len,
                                    Command **out_cmd) {
  size_t i = 0;
...
int command_reader_read_cmd(CommandReader *r, Command **out_cmd) {
  ...
  *out_cmd = NULL;
  ...
  while (start < acc.len && isspace((unsigned char)acc.data[start]))
    start++;
