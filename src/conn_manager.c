...
/* Returns the ConnEntry identified by connection_name or NULL. Since for now we expect
 * few connection (<50 surely) this is good enough even for O(n) search time.
 * TODO: we should use an hashmap. */
static ConnEntry *find_entry(ConnManager *m, const char *connection_name) {
    if (!m || !connection_name) return NULL;
    for (size_t i = 0; i < m->n_entries; i++) {
      const ConnProfile *p = m->entries[i].profile;
        if (p && p->connection_name && strcmp(p->connection_name, connection_name) == 0) {
            return &m->entries[i];
        }
    }
    return NULL;
...
static void connm_disconnect_idle(ConnManager *m) {
    if (!m || !m->entries || m->ttl_ms == 0) return;

    uint64_t t = now_ms_monotonic();
    for (size_t i = 0; i < m->n_entries; i++) {
        ConnEntry *e = &m->entries[i];
        if (!e->backend || db_is_connected(e->backend) != YES) continue;
